...
- **`.github/workflows/deploy.yml`** — só em push na `main` (depois do CI passar): builda as duas imagens (`web`, `server`) com `docker/build-push-action`, publica no GitHub Container Registry (`ghcr.io/<owner>/gerador-web`, `ghcr.io/<owner>/gerador-server`, grátis e já autenticado via `GITHUB_TOKEN`, sem conta em mais um serviço), depois via SSH (`appleboy/ssh-action`) copia `docker-compose.prod.yml` + `Caddyfile` pra `/opt/gerador` na VM e roda `docker compose -f docker-compose.prod.yml -f docker-compose.secrets.yml pull && ... up -d`.
