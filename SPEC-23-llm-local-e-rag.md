...
### Correção pós-1d (implementada): a fila era placeholder por placeholder — faltava história e teste contextual, o pedido original desde o início

Usuário testou `v0.1.28` num cenário real (aprovação de crédito, 13 itens) e reportou dois problemas: (1) a tela pulou direto pro estado final sem nenhuma animação/barra de fase, e (2) mesmo que rodasse, o mecanismo de hoje só preenche checklist técnico/volumetria — não escreve história de usuário nem cenários de teste contextuais, que é o pedido original desde a Fase 1 (§6.5: "o usuário construía os itens estruturados... usando IA", inspirado no fluxo de colar a especificação num chat de IA externo).

Investigado antes de desenhar: (1) confirmado — sem placeholder pendente (regras.json sem regra pra aquelas techs/contextos, ou modelo não instalado nesse teste), a régua "0 pendências = refinado" marca tudo como pronto trivialmente, sem nunca disparar `useGeracaoAoVivo`. (2) confirmado e mais grave: `historiaPo` é um **texto fixo hardcoded** (`gerarEspecificacaoEntrega.ts:476`, `"Como <papel>, quero <ação> para que <benefício — detalhar>."`), nunca tocado por IA — mas é um campo por-quebra (visão geral do documento), não por-item. O que o usuário quer por item — a história do item + cenários de teste — hoje é `atividade.descricao` (frase mecânica gerada por `derivar()`, tipo "srv-checkout publica em fila-pedidos", não uma história de usuário) e `criteriosAceiteMarkdown` (scaffold Gherkin determinístico por tipo de nó, nunca contextualizado).

Pedido do usuário, direto: um pipeline simples — a IA recebe todo o contexto do épico + todas as informações do item, devolve num **formato padrão** (estruturado), disparado automaticamente após "Derivar" — e só depois disso existir, uma segunda camada ("editar com IA", em chat, individual ou em lote) permite revisar/ajustar. A segunda camada é exatamente o que já estava desenhado como 1e ("funil unificado de proposta") — fica confirmada como o próximo passo depois desta correção, não descartada.

**Decisão de arquitetura**: a fila deixa de ser um passo por *placeholder* e vira um passo por **item** — cada passo faz UMA chamada de IA que devolve, de uma vez, história de usuário + critérios de aceite contextuais + todo o checklist técnico/volumetria pendente daquele item, via schema JSON **dinâmico** (as chaves do schema são geradas por item, a partir de `listarPlaceholders()` — o schema não é fixo em compile-time, é montado por chamada, o que `completarComSchema` já suporta desde sempre). Isso volta a usar GBNF (estrutura), não o texto livre da Fase 1c — decisão consciente: pra um call multi-campo, a garantia de shape vale mais que o streaming caractere-a-caractere; o streaming continua existindo pro botão manual "✨ Sugerir" de um placeholder isolado (`/ia/sugerir`, inalterado). O card do item mostra "✨ gerando a ficha inteira..." enquanto a chamada única está em voo, sem crescimento de texto ao vivo — trade-off registrado, não uma regressão silenciosa.

**Mudanças**:
- `packages/engine/src/refinamento/gerarRefinamento.ts`: exporta `respostaVisivel`; `PlaceholderRefinamento.secao` ganha `"historiaUsuario" | "criteriosAceite"`; `listarPlaceholders()` sempre inclui esses dois placeholders no início (chaves fixas `_historiaUsuario`/`_criteriosAceite`, sem depender de `regras.porTech` — toda atividade precisa de história, independente de ter regra técnica ou não).
- `packages/engine/src/especificacao/gerarEspecificacaoEntrega.ts`: `renderizarItemEspecificacao` ganha uma seção "História de usuário" (usa a resposta confirmada, cai no marcador `<- ✍️ especificar` sem ela) e acrescenta os critérios de aceite confirmados depois do scaffold Gherkin ("Cenários adicionais (contextuais)"). `FichaItem` ganha `historiaUsuario`/`criteriosAceiteContextual: FichaPlaceholder`, populados por `montarFichaItem` do mesmo jeito que `checklistTecnico`/`volumetria` já são.
- `packages/cli/src/commands/openApiLocal.ts`: rota nova `POST /ia/sugerir-item` — recebe `{contextoEpico?, contextoNo, atividadeRotulo, placeholders: {chave,tech,rotulo}[]}`, monta o schema JSON dinâmico (`{type:"object", properties: {[chave]: {type:"string"}}, required: [...chaves]}`), chama `completarComSchema`, devolve `Record<chave,string>`. `/ia/sugerir` continua existindo, sem mudança — é o que o botão manual por placeholder usa.
- `packages/web/src/api/client.ts`: `apiIa.sugerirItem(pedido): Promise<Record<string,string>>`.
- `packages/web/src/review/useGeracaoAoVivo.ts`: fila vira `{atividadeChave, contextoNo, placeholders}[]` (um item = uma atividade, todos os placeholders pendentes juntos); cada passo chama `sugerirItem` uma vez e distribui as respostas via `onResponderItem` por chave.
- `packages/web/src/review/ReviewScreen.tsx`: `montarFila` inclui `historiaUsuario`/`criteriosAceiteContextual` junto de `checklistTecnico`/`volumetria` na lista de pendentes; fase bar passa a contar "item N de M" (não mais "requisito"); `AbaRefinamento` mostra os dois novos placeholders no topo da lista (mesma UI de sempre — Sugerir/Confirmar manuais continuam funcionando por placeholder isolado via `/ia/sugerir`).

**Fora de escopo, deliberado**: o "editar com IA" em chat (individual ou em lote, com revisão de consistência entre itens) — isso é 1e, próximo passo confirmado depois desta correção, não construído agora; streaming ao vivo do call multi-campo (fica só o indicador "gerando a ficha inteira", sem crescimento de texto — resolver isso exigiria parsear JSON parcial, especulativo sem necessidade real ainda).

**Feito quando**: depois de "Derivar Quebra" com o modelo instalado e regras configuradas pra pelo menos uma tech usada no cenário, cada item recebe história de usuário + critérios de aceite contextuais + checklist técnico numa única chamada de IA, visível no documento final exportado; validado com Playwright contra o modelo real; regressão completa verde.

**Validação real (Playwright + Qwen3-4B de verdade, cenário "Mensageria RabbitMQ", 6 itens)**: `gerador open` num projeto novo, cenário carregado pela Jornada, "Derivar Quebra" disparou a orquestração sozinha — fase bar mostrou "Escrevendo item 1 de 6 · 01" (não mais "requisito"), os três placeholders da ficha (História de usuário / Critérios de aceite / checklist técnico) mostraram "✨ gerando a ficha inteira…" simultaneamente enquanto a chamada única estava em voo. Depois do item 01 concluir, a ficha mostrou conteúdo real e contextual, não genérico — ex.: história "Como membro do time de antifraude, quero receber uma notificação assíncrona quando um pagamento for aprovado par[a]..." e critério "Ao receber um pagamento aprovado, o sistema deve enviar uma mensagem para o time de antifraude via DLQ dentro..." — citando nomes reais dos nós do diagrama (`srv-antifraude`, `pagamento.aprovado.q`), confirmando que o gap arquitetural original (histórias/critérios nunca contextualizados por IA) está fechado. Achado de desempenho, registrado sem ser bloqueante: o call multi-campo via GBNF é mais lento que o streaming de campo único da Fase 1c (~1-1.5min por item nesta máquina) — pra uma quebra de 13 itens como a do relato original do usuário, o tempo total de geração ao vivo pode passar de 15min; aceitável pra v1 (o usuário não precisa esperar — pode navegar/editar manualmente enquanto a fila roda ao fundo), mas é candidato a otimização (paralelismo por item, modelo menor, ou streaming parcial de JSON) se virar fricção real.
