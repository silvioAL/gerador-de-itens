...
describe("RegrasTab (SPEC-23 fluxo 5 — regras.json ganha tela)", () => {
  ...
  it("adicionar um requisito grava o arquivo inteiro preservando processo/testes da tech", async () => {
    render(<RegrasTab />);
    await waitFor(() => expect(screen.getByLabelText("Novo requisito")).toBeTruthy());

    fireEvent.change(screen.getByLabelText("Novo requisito"), {
      target: { value: "Definir a chave de idempotência da mensagem" },
    });
    fireEvent.change(screen.getByLabelText("Contextos do novo requisito"), { target: { value: "Backend-mensagens" } });
  ...
  it("remover tira só o requisito escolhido", async () => {
    render(<RegrasTab />);
    await waitFor(() => expect(screen.getByLabelText("Remover requisito 1")).toBeTruthy());
    fireEvent.click(screen.getByLabelText("Remover requisito 1"));
  ...
  it("trocar de tech mostra a lista da outra tech", async () => {
    ...
    await waitFor(() => expect(screen.getByText("Nenhum requisito para esta tecnologia ainda.")).toBeTruthy());
  });
});
