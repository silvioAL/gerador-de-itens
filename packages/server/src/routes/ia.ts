...
export async function registrarRotasIa(app: FastifyInstance, { db }: OpcoesApp) {
  ...
  // §356 → SPEC-107 G3: `POST /ia/documento-externo` MORREU — a conversa lê a
  // página pelo executor genérico de conector (`POST /conectores/:id/executar`,
  // que o catálogo em vigor resolve pelo MESMO destino). A régua do §349 §6
  // ("200 com conteúdo vazio é o mesmo que não achar") mudou de casa: vive na
  // tela, sobre `{saida, ausentes}` — com a mesma mensagem.
