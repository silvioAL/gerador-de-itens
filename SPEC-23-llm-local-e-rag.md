...
- **1a — Dado estruturado por atividade no engine**: hoje só existe a string markdown (`renderizarItemEspecificacao`). Pré-requisito de tudo mais: expor a MESMA informação como objeto estruturado (título, descrição, critérios de aceite, contrato request/response, requisitos com origem/valor, testes) — o markdown vira uma projeção desse objeto, não a fonte.
- **1b — Contexto do épico**: `Quebra` ganha campo(s) pra texto longo + anexos, tela dedicada antes de "Gerar", persistência local (arquivo, mesmo padrão de `quebras/<id>.json`).
- **1c — Streaming real**: `motor.ts` já suporta `onTextChunk` (`completar()`), não usado ainda — nova rota com streaming de verdade (chunked/SSE, diferente do request-response simples de `/ia/sugerir`), com a mitigação de retry-em-`�`.
- **1d — Tela de geração**: a experiência do protótipo (fase, diagrama animado, itens pousando, ficha em abas), consumindo o streaming real de 1c sobre o dado estruturado de 1a.
