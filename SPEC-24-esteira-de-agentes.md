...
**Encadeamento + re-rodar validados com Playwright contra o `gerador open` real, Qwen3-4B ao vivo** (dois cenários, `confirmacaoObrigatoria: false`): a cascata de insumos acumulou exatamente como desenhado — PO com `respostasAnteriores: []`, Arquiteto recebendo história+critérios, Especialista recebendo +5 campos do contrato, QA recebendo tudo (+requisito técnico). Critérios de aceite saíram **numerados** (lista "1./2./3.") citando restrições do épico ("aproximado", "5 minutos") — antes eram 2-3 linhas de prosa. **Re-rodar a partir da alteração**: após a esteira terminar, o botão "↻ Re-rodar papéis seguintes" da seção PO disparou exatamente 3 chamadas (`arquiteto, especialista, qa` — sem PO), com 1 item só, todas com a história como insumo via `respostasAnteriores`. Nota honesta de limite: com 4B, a quantidade de critérios fica no piso da faixa (3) e há escorregões semânticos ocasionais — exatamente o teto de capacidade que a SPEC-25 (seleção de modelo) endereça.
