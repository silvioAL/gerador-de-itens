...
2. **SPEC-25 Fase 1 (DeepSeek local)** — promovida: com a barra calibrada em "nível DeepSeek" (SPEC-25 §4.3), este é o modelo que provavelmente **atende de verdade** na empresa, e o gargalo nomeado (raciocínio) não se resolve com prompt. Vem cedo também para **desarriscar**: tudo abaixo precisa rodar nele, e descobrir isso no fim custaria retrabalho.
3. **Bloco 1** — procedência + obsolescência. Determinístico, barato, pré-requisito dos blocos 2/3 e com valor imediato: a tela passa a avisar o que ficou para trás mesmo sem IA nenhuma.
4. **Bloco 4a** — checagens determinísticas do engine. Independem de tudo, custam pouco, atacam direto o "não esquecer coisas".
5. **Blocos 2 + 3** — propagação e diff, casados. Desenhados no Claude, **validados também no embarcado** antes de fechar (princípio de §8.2). O diff é justamente o que torna aceitável o modelo errar.
