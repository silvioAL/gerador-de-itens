...
3. **`CamposArestaTab`/`CamposNoTab`: "Ajuda" confundível com o rótulo, e "sobrescrever" parecia não fazer nada.** Achados de UX (JOURNEY.md §54), não bug de dado: legenda curta adicionada abaixo de "Valor padrão"/"Ajuda" explicando o que cada um faz; `scrollIntoView` no container do formulário disparado por `abrirNovo`/`abrirEdicao`/`abrirSobrescrita`, já que o formulário abre sempre no topo da aba mas os gatilhos ficam na lista, mais abaixo. Só `edgeTypes.http` tinha um campo de exemplo (`timeoutMs`, §2) — os outros 12 tipos ficavam sempre "Nenhum campo ainda." até alguém adicionar o primeiro à mão. Adicionados exemplos ilustrativos em `grpc` (`timeoutMs`), `publishes`/`pubsub` (`chaveRoteamento`) e `consumes` (`sincrono`), em `config/diagrama.example.json` e no mirror `packages/cli/templates/diagrama.json`.
