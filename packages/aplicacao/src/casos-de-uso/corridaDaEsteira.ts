...
export async function correrEsteiraPelaFila(opcoes: {
  ...
  const { fila, papeisAtivos, contextoEpico, contextoDoProduto, completarEstruturado } = opcoes;
  // O acumulador nasce das respostas que JÁ existiam (confirmadas/edições) —
  // exatamente como o hook (o snapshot é dele; a referência viva cresce).
  ...
  for (const papel of papeisAtivos) {
    ...
    for (let i = 0; i < doPapel.length; i += TAM_LOTE_ESTEIRA) {
      ...
      try {
        ...
        const bruto = (await completarEstruturado(papel.id, pedido.prompt, pedido.esquema)) as Record<
          string,
