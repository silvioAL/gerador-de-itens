...
- `GET/POST/PUT /quebras` → um único `quebra.json` na raiz do projeto — **o mesmo arquivo** que `gerador derive`/`implementar` já esperam como argumento, fechando o ciclo entre desenhar no canvas e rodar via terminal.
...
- `GET /campos-no` sempre `[]`, e qualquer escrita em `/campos-no`, `/times`, `/convites` devolve 501 com mensagem explicando por quê — simplificação deliberada: campo customizado por time e múltiplos times são conceitos do modo hospedado, sem equivalente local (edição de spec de nó continua sendo direto em `config/diagrama.json`).
