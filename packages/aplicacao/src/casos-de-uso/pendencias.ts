...
/**
 * SPEC-44 — a contagem de pendências, pura. É a régua ÚNICA: a barra, os
 * chips dos cards e a seção dos itens do documento falam a partir daqui —
 * "sugestão aguardando" (a esteira escreveu, ninguém assinou) e "campo vazio"
 * (ninguém escreveu) são coisas diferentes e a frase diz qual.
 *
 * SPEC-107 G5c — mudou de casa (era `web/src/review/pendencias.ts`): o
 * julgamento campo a campo vive na DEMANDA (§5.5), e a superfície que o
 * hospeda — hoje a revisão, amanhã a seção dos itens do documento — importa a
 * MESMA régua daqui (§263).
 */

export { respostaConfirmada };

/** Os placeholders da ficha, achatados na ordem das seções (PO, Arquiteto,
 * Especialista, QA) — mesma ordem de `placeholdersDaFichaPorGrupo`. */
